...
namespace Tests
{
    ...
    public class SettingsParserTests
    {
        ...
        {
            SettingsParser sp = new SettingsParser();
            Dictionary<string, string> settings = sp.Parse(ServerSettings.ReadSettings(), "icevm069");

            Assert.IsNotNull(settings);
            Assert.AreEqual(5, settings.Keys.Count);
            Assert.AreEqual(@"icevm069", settings["server.name"]);
            Assert.AreEqual(@"d", settings["local.root.drive.letter"]);
            Assert.AreEqual(@"_releasetemp", settings["deployment.working.folder"]);
            Assert.AreEqual(@"d:\_releasetemp", settings["local.temp.working.folder"]);
            Assert.AreEqual(@"\\icevm069\_releasetemp", settings["remote.temp.working.folder"]);
        }
    }
}
